...
/**
 * Offers the partner moments to `goldnead/statamic-webhook-manager` as
 ...
 * that dispatches in whatever brand is current, and a commission is booked in a
 * payment webhook where none is; the manager's hooks are brand-scoped.
 *
 * **Nothing here loads a class of the webhook manager before its name has been
 * checked as a string.** {@see AffiliatesTrigger} implements the manager's
...
class WebhookManagerBridge
{
    ...
    {
        return (bool) config('affiliates.webhook_manager.enabled', true)
    ...
    public function boot(Dispatcher $events): void
    {
        ...
        foreach (WebhookPayload::MOMENTS as $moment => $eventClass) {
            try {
                $manager->registerTrigger(new AffiliatesTrigger($moment));
            } catch (Throwable $e) {
                Log::warning('statamic-affiliates: the webhook manager would not take the trigger ['.WebhookPayload::PREFIX.$moment.'].', [
    ...
    /**
     * Never throws: a receiver that is down must cost a webhook, not a
     * commission.
     */
    protected function dispatch(string $moment, object $event): void
    {
        ...
        // After the commit, never inside it. The ledger reverses commissions
        // inside a transaction: handed over there, a queued delivery could run
        // before the rows exist, a sync one would hold the write lock for an
        // HTTP round trip, and a rollback would leave the receiver told about a
        // reversal that never happened. Outside a transaction this runs at
        // once; after a rollback never.
        try {
            DB::afterCommit(fn () => $this->handOver($handle, $event));
        } catch (Throwable $e) {
            Log::warning('statamic-affiliates: a partner moment could not be handed to the webhook manager.', [
                'trigger' => $handle,
                'exception' => $e->getMessage(),
            ]);
        }
